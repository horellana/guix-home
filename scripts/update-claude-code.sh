...
# reescribe (version ...) y (base32 ...) en las dos copias de claude-code.scm
# (la de este repo y la del canal local que lee Guix).  Termina validando con
# `guix home build`; la activación (reconfigure) y el commit quedan a mano.
set -euo pipefail

repo=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
src="$repo/claude-code.scm"
channel=/home/hector/guix-packages/my-packages/claude-code.scm
